...
export default function FacturaDetalleAdmin() {
    ...
    <div className="max-w-5xl mx-auto p-8">
      ...
      <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8">
        ...
        <div className="flex justify-between items-start mb-8 pb-6 border-b border-gray-200">
          <div>
            ...
            <p className="text-sm text-gray-500 mt-1">
              Estado de orden: <span className="font-medium">{factura.orden.estado}</span>
            </p>
          </div>
        ...
        <div className="mb-8">
          ...
          <div className="bg-gray-50 rounded-lg p-4 grid md:grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-gray-900 font-medium">{factura.orden.user_name}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600 mb-1">Email</p>
              <p className="text-gray-900 font-medium">{factura.orden.user_email}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600 mb-1">ID de usuario</p>
              <p className="text-gray-900 font-medium">#{factura.orden.user_id}</p>
            </div>
            <div>
              <p className="text-sm text-gray-600 mb-1">Método de pago</p>
              <p className="text-gray-900 font-medium">{factura.metodo.toUpperCase()}</p>
            </div>
          </div>
        </div>
                  ...
                  <tr key={item.id}>
                    <td className="px-6 py-4">
                      <div>
                        ...
                        <p className="text-xs text-gray-400 mt-1">ID: {item.producto.id}</p>
                      </div>
                    </td>
        ...
        <div className="mt-8 pt-6 border-t border-gray-200">
          <h2 className="text-lg font-semibold text-gray-800 mb-3">Información de transacción</h2>
          <div className="bg-gray-50 rounded-lg p-4 space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">ID de factura:</span>
              <span className="font-mono text-gray-900">#{factura.id}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">ID de orden:</span>
              <span className="font-mono text-gray-900">#{factura.orden_id}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">ID de Stripe:</span>
              <span className="font-mono text-xs text-gray-900">{factura.stripe_id}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">Fecha de creación de orden:</span>
              <span className="text-gray-900">{new Date(factura.orden.created_at).toLocaleString()}</span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-gray-600">Última actualización:</span>
              <span className="text-gray-900">{new Date(factura.orden.updated_at).toLocaleString()}</span>
            </div>
          </div>
        </div>
      </div>
